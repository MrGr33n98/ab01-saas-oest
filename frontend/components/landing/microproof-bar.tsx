      ...
      <div className="mx-auto grid max-w-[1320px] grid-cols-2 px-6 lg:grid-cols-4 lg:px-8">
        {proofPoints.map(([title, detail], index) => (
          <div
            ...
            key={title}
          >
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-oest-blue">0{index + 1}</p>
            <p className="mt-1 text-sm font-semibold text-oest-ink">{title}</p>
            <p className="mt-1 max-w-[210px] text-[12px] leading-relaxed text-oest-ink/65">{detail}</p>
          </div>
        ))}
      </div>
    </section>
