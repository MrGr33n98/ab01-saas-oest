  ...
  return (
    <section className="bg-oest-blue py-24 text-white sm:py-32">
      <div className="mx-auto max-w-[1080px] px-6 lg:px-8">
        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-oest-yellow">Perguntas frequentes</p>
        <h2 className="mt-5 max-w-[850px] text-[42px] font-bold leading-[0.96] tracking-[-0.045em] sm:text-[56px]">O que você precisa saber para planejar a próxima missão.</h2>
        <div className="mt-14 border-y border-white/25">
          {faqs.map(([question, answer], index) => {
            const isOpen = open === index;
            return <article className="border-b border-white/25 last:border-b-0" key={question}>
              <button aria-expanded={isOpen} className="flex min-h-16 w-full items-center justify-between gap-8 py-5 text-left" onClick={() => setOpen(isOpen ? null : index)} type="button">
                <span className="text-[16px] font-medium leading-snug sm:text-[17px]">{question}</span>
                <span aria-hidden className="text-[24px] font-light leading-none">{isOpen ? "−" : "+"}</span>
              </button>
              <div className={`grid transition-[grid-template-rows] duration-300 ${isOpen ? "grid-rows-[1fr] pb-5" : "grid-rows-[0fr]"}`}><div className="overflow-hidden"><p className="max-w-[760px] text-[14px] leading-relaxed text-white/78">{answer}</p></div></div>
            </article>;
          })}
        </div>
      </div>
